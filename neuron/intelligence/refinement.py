...
from .utils import extract_json_from_response, get_last_user_content, format_chat_prompt


async def _refined_axis(conv, axis_name, axis_model):
...
async def _refine_axes(conv):
    result_coroutines = []

    for axis_name, axis_model in AXIS_REGISTRY:
        result_coro: Coroutine = _refined_axis(conv, axis_name, axis_model)
